...
const NAV_ROOTS = ['/home', '/catalog'];

export default function Header({ username, hasSidebar }: HeaderProps) {
  const pathname = usePathname();

  const showFullNav = NAV_ROOTS.some((r) => isUnder(pathname, r))

  return (
      <header
          className={`py-4 px-6 ${showFullNav ? 'bg-white border-b border-gray-200' : 'bg-gray-100'}`}
      ...
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <input
                  ...
                  className={`pl-10 pr-4 py-2 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-slate-500 w-64 ${
                      showFullNav ? 'bg-gray-100' : 'bg-white'
                  }`}
              />
            ...
            <button className={`p-2 rounded-lg transition-colors ${showFullNav ? 'hover:bg-gray-100' : 'hover:bg-gray-200'}`} aria-label="Notifications">
